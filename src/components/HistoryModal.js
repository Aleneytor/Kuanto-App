...
import { X, History, Calendar, CalendarSearch, Download, ChevronLeft, AlertTriangle } from 'lucide-react-native';
...
const HistoryModal = ({ visible, onClose, rates, usdtHistory, onOpenDatePicker }) => {
    ...
    // Reset view and trigger animation when opening/closing
    React.useEffect(() => {
        if (visible) {
            setView('list');
            setLoading(false);
    ...
    // Limit the displayed list to the latest 14 days
    const filteredData = historyData.slice(0, 14);
            ...
            <FlatList
                ...
                ListFooterComponent={() => (
                    historyData.length > 14 ? (
                        <TouchableOpacity 
                            ...
                            onPress={() => {
                                ...
                                if (onOpenDatePicker) {
                                    setTimeout(() => {
                                        onOpenDatePicker();
                                    }, 350); // wait for modal closing animation
                                }
                            }}
                            style={{ 
                        ...
                        >
                            <CalendarSearch size={32} color={colors.textSecondary} opacity={0.7} strokeWidth={2.2} />
                            <Text style={{ 
                                fontSize: 13, 
                                ...
                                Presiona aquí para abrir el calendario y escoger una fecha específica para ver las tasas de ese día
                            </Text>
                        </TouchableOpacity>
                    ) : null
                )}
    ...
    return (
        <Modal
            visible={visible}
            animationType="none" // Controlled manually for softer feel
            transparent={true}
