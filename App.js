...
function AppContent() {
  ...
  React.useEffect(() => {
    ...
    const checkInitialSetup = async () => {
      try {
        ...
        // Defer heavier operations so they don't block the initial JS thread
        setTimeout(async () => {
        ...
        }, 1500); // 1.5s delay gives priority to rendering
      } catch (e) {
        console.warn("Error in initial setup:", e);
  ...
  if (isFirstLaunch === null || themeLoading) {
    // We render an empty view with the same background as the splash to avoid flickering
    // expo-splash-screen is currently visible over this
      ...
      >
        <Stack.Navigator
          ...
          screenOptions={{ 
            ...
            animation: 'fade', // Soft global transition
          }}
        >
...
export default function App() {
  ...
  // Hide splash screen handled inside AppContent, but just in case
  if (!fontsLoaded) {
    return <View style={{ flex: 1, backgroundColor: '#1c1c1e' }} />;
  }
